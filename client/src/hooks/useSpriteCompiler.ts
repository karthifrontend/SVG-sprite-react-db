    ...
    async (
      ...
      try {
        ...
        const existingSymbols = options?.existingContent
          ...
          : [];
        // Pre-compute the duplicate set against the base sprite so we can return a summary the caller uses for its toast copy and to short-circuit the all-duplicates case. We compare by symbol id (derived from the staged file's name) — the same id the server-side library merge uses, so a file dropped here that collides with an existing library symbol is treated as a duplicate here too.
        // The short-circuit (`allDuplicates: true`) means: if every staged file produced an id that already lives in the base sprite, we skip the merge / blob / state updates entirely. The caller (`handleGenerate`) then bails before touching the base-sprite file, the inline save state, or the library save flow — there's nothing to save, and creating a "new version" of the sprite with zero net new icons would be misleading.
