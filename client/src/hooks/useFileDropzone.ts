...
export function useFileDropzone(options?: {
  ...
  // Fired once per file that was filtered out because it didn't match the dropzone's `accept` mode. Non-SVG files are NOT reported here — they're silently dropped, since users routinely drop the wrong thing and we don't want to nag.
  onRejected?: (rejected: RejectedFile) => void;
}) {
  const acceptMode: DropzoneAcceptMode = options?.accept ?? "icons";
  const [files, setFiles] = useState<File[]>([]);
  const inputRef = useRef<HTMLInputElement>(null);
  // Keep the option callbacks in refs so `addFiles` doesn't need to re-create itself on every render.
  ...
  const addFiles = useCallback(async (incoming: FileList | null) => {
    ...
    // Classify each SVG against the dropzone's accept mode. We do this serially because `isSpriteSvgFile` reads the file via `File.text()` and we want the toasts / order to match what the user dropped.
    const mode = acceptRef.current;
    const classified: Array<{ file: File; isSprite: boolean }> = [];
    for (const file of svgOnly) {
      // `isSpriteSvgFile` swallows read/parse errors and returns `false`, so a corrupt SVG is just treated as "not a sprite" rather than throwing into the dropzone flow.
      const isSprite = await isSpriteSvgFile(file);
      classified.push({ file, isSprite });
    }
    const accepted: File[] = [];
    for (const { file, isSprite } of classified) {
      // Filter by accept mode. The `kind` we report to the caller is the kind we REJECTED — the file the user should have dropped in the OTHER section.
      const matches =
    ...
    setFiles(prev => {
      // Build a lookup of files already staged. Use name + size as the dedupe key (size disambiguates identically-named files of different content).
  ...
  // Append already-constructed File objects to the staged list. Mirrors `addFiles` but skips the FileList -> array conversion and the SVG MIME filter, so callers that produced the File objects themselves (e.g. the "paste into workspace" flow, which builds a `File` from a copied `<svg>` string) can drop them straight in. The filter is intentionally relaxed to accept any `image/svg+xml` file — pastes always come with the right MIME since the LiveDemo constructs them explicitly.
  ...
  // Remove a specific set of staged files by reference. Used by the "Undo paste" flow in the live demo, which needs to take back the exact files it just appended even if the user dropped or removed other files in the meantime. We compare by File identity (the same object reference) so files pasted earlier with the same name aren't accidentally pulled out.
