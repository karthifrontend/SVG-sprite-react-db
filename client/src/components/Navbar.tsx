  ...
  return (
    <nav className="fixed left-0 right-0 top-0 z-40 flex items-center justify-between border-b border-slate-200/60 bg-white/80 px-4 py-3 backdrop-blur-md">
      ...
      <div className="flex items-center gap-3">
        {currentUser ? (
          <>
            <span className="rounded-md bg-slate-100 px-3 py-1.5 text-sm text-slate-700">
              {currentUser.email}
            </span>
            <button
              type="button"
              onClick={handleLogout}
              className="text-sm font-medium text-slate-700 hover:text-rose-600 transition-colors px-3 py-1.5 bg-transparent"
            >
              Logout
            </button>
          </>
