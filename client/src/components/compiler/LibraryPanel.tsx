...
// Confirmation modal for deleting an entire library (every version). Mirrors the styling of the per-version delete modal but spells out that ALL versions will be removed and the bundle will disappear from the panel and from MongoDB.
    ...
    >
      <div className="p-6">
        ...
        <p className="text-xs text-slate-600">
          You are about to delete the entire library{" "}
          <span className="font-semibold text-slate-900">“{bundleName}”</span>{" "}
          and all of its{" "}
          <span className="font-semibold text-slate-900">
            {versionCount} version{versionCount === 1 ? "" : "s"}
          </span>{" "}
          from your library. This action is permanent.
        </p>
...
function LibraryPanel({
  ...
  // Pending library (bundle) delete: the whole group the user wants to remove. Confirmed via a separate modal so the user explicitly opts in to dropping every version at once.
  const [pendingBundleDelete, setPendingBundleDelete] = useState<{
    id: string;
    bundleName: string;
    versionCount: number;
  } | null>(null);
  const [bundleDeleteBusy, setBundleDeleteBusy] = useState(false);
            ...
            <div className="mb-2 flex items-center gap-1.5">
              ...
              {group.isOwner && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    openBundleDeleteModal(group);
                  }}
                  className="inline-flex shrink-0 items-center justify-center rounded p-1 text-slate-400 transition-colors hover:bg-rose-50 hover:text-rose-500"
                  title={`Delete library “${group.bundleName}” and all its versions`}
                  aria-label={`Delete library ${group.bundleName}`}
                >
              ...
              {group.versions.length > COLLAPSED_VERSION_COUNT && (
                <button
                  ...
                  className="inline-flex shrink-0 items-center justify-center rounded p-1 text-slate-400 transition-colors hover:bg-indigo-50 hover:text-indigo-600"
                >
                  {expandedGroupNames.has(group.bundleName) ? (
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center justify-between gap-1.5">
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-1 rounded bg-indigo-50 px-6 py-1 text-[11px] font-semibold text-indigo-600 transition-colors hover:bg-indigo-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500 disabled:opacity-60"
                        title={
                          version.isOwner
                            ? undefined
  ...
  function openBundleDeleteModal(group: LibraryGroup) {
    if (group.versions.length === 0) return;
    setPendingBundleDelete({
      // Pass the id of the latest version — the server's `?scope=bundle` resolves the whole bundle from any one of its versions.
      id: group.versions[0].id,
  ...
  async function handleConfirmBundleDelete() {
    if (!pendingBundleDelete) return;
    setBundleDeleteBusy(true);
    try {
      const removedName = await deleteBundle(pendingBundleDelete.id);
      onLibraryDeleted?.({ name: removedName });
      setPendingBundleDelete(null);
    } catch (err) {
      showToast(
  ...
  return (
    ...
    >
      ...
      />
      <DeleteBundleModal
        open={!!pendingBundleDelete}
        bundleName={pendingBundleDelete?.bundleName ?? ""}
        versionCount={pendingBundleDelete?.versionCount ?? 0}
        busy={bundleDeleteBusy}
        onCancel={() => {
          if (!bundleDeleteBusy) setPendingBundleDelete(null);
        }}
