...
// Sprite bundle metadata. One document per logical library (slug). The actual XML / symbolIds / version content lives in the `sprite_versions` collection — this document only carries the shared properties of a bundle: its unique slug, who owns it, whether it's public, the latest version number, and how many symbols that latest version contains.
const spriteSchema = new Schema(
  {
    // Slug used to group versions into a single library. Combined with `ownerId` it must be unique: two owners can each have a library called "icons" without colliding, but one owner cannot have two "icons" bundles. The slug is sanitised on write (trim + collapse whitespace) by the route layer.
    bundleName: {
      type: String,
      required: true,
      trim: true,
    ...
    // The Google-authenticated user that owns this bundle. Set on create from the bearer-token session; used to scope reads and to gate edits / renames / deletes. Indexed for the owner-scoped "list my libraries" query.
    ownerId: {
    ...
    // The most recent version number across all SpriteVersion rows pointing at this bundle. Bumped every time a new version is inserted. Used as the "next version" hint on save and as the latest-version indicator in the library list (so the UI can label the newest row "v3" without a join).
    currentVersion: {
      type: Number,
      required: true,
      default: 1,
    ...
    // Convenience copy of the latest version's symbol count. Kept on the bundle doc so the library list endpoint can return summary rows without pulling in any SpriteVersion data.
    symbolCount: {
      type: Number,
      default: 0,
    },
    // When true, every authenticated user can see this bundle in the library list and load the latest version. Edits, renames and deletes are still restricted to `ownerId` server-side.
    isPublic: {
      type: Boolean,
      default: false,
      index: true,
...
// One bundle name per owner. This is the integrity rule the rename-collision check enforces: a given user can never end up with two "icons" bundles even if the slug casing differs.
...
// Resolve the bundle `_id` for a hook invocation. For the document middleware the id lives on `this`; for the query middleware it lives in `getFilter()`. Returns `undefined` when neither is available, in which case the caller should skip the cascade and let the underlying delete proceed.
...
// Issue the version cascade. Throws on failure so Mongoose's promise hook machinery surfaces the error and aborts the parent delete.
...
// Combined document + query hook for `deleteOne`. Mongoose 9 supports promise-returning middleware, so the hook body just resolves the bundle id, runs the cascade, and lets thrown errors propagate. Mongoose aborts the delete on a rejection.
spriteSchema.pre(
  "deleteOne",
  ...
  async function preDeleteOneCascade(this: unknown, doc: unknown) {
    ...
    if (idField === undefined) {
      // No id in the filter — we can't safely cascade. Let the delete proceed; the route layer always filters by `_id`.
      return;
    }
